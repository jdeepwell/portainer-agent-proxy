...
The nginx configuration manager is implemented in `app/nginx_manager.py`. It validates mapping inputs, resolves the active upstream client certificate paths, generates deterministic per-port HTTPS nginx server blocks with managed metadata comments, parses managed nginx config files back into mapping data, references the inbound server certificate persisted under `/data/server-certs/`, validates candidate config sets with `nginx -t` before writing live files, performs atomic writes for generated or pre-rendered config content under `/data/nginx/conf.d/`, supports validated deletes, installs uploaded client certificate/key pairs under `/data/certs/`, and reloads nginx with shell-free subprocess calls. Focused unit tests cover mapping validation, active certificate resolution, HTTPS config generation and parsing, write/delete behavior, failed validation behavior, uploaded certificate installation, and reload command execution.
...
The Flask management API is implemented in `app/main.py`. It serves the management UI shell and health endpoint, exposes mapping list/add/delete/ping REST routes, exposes certificate status/upload routes, reads mappings directly from managed nginx config files under `/data/nginx/conf.d/`, auto-assigns ports from `9101`, delegates nginx writes, deletes, and certificate installs to the privileged agent over `/run/nginx-agent.sock`, and has no separate JSON mapping store. The ping endpoint uses Python's standard HTTPS client with the active client certificate/key pair and treats HTTP error responses as reachable remote responses rather than transport failures; `403 Forbidden` from a Portainer Agent is reported as an OK reachability result because the UI probe is expected to be unsigned. API tests cover config-backed mapping listing, automatic port selection, duplicate rejection, certificate status/upload behavior, agent failure handling, delete behavior, ping responses including expected `403`, socket communication, and client-certificate setup.
...
Container and deployment integration is implemented. The repository includes `compose.example.yml` for Portainer-side deployment, `README.md` documents GHCR image usage, mapping storage in `/data/nginx/conf.d/`, upstream client certificate persistence, inbound HTTPS certificate persistence, and Portainer Agent addresses using `portainer_mtls_proxy:<port>` or `https://` when a full URL is required. `.github/workflows/docker-image.yml` builds, tests, tags, and publishes the image to GHCR on `main`, semantic version tags, pull requests, and manual dispatch.
...
- The image creates required runtime directories (`/data`, `/data/nginx/conf.d`, `/data/server-certs`, `/run`, and `/certs`) and runs `supervisord` as PID 1.
- The runtime entrypoint prepares `/data/nginx/conf.d` as a root-owned readable config directory, generates the persisted inbound HTTPS certificate/key pair when missing, validates that user-supplied inbound cert files are complete, and then starts supervisord.
...
- Generated proxy config listens with HTTPS on each mapping port, presents `/data/server-certs/proxy.crt` and `/data/server-certs/proxy.key` to Portainer, stores the mapping name in a managed metadata comment, and enables upstream TLS SNI with `$proxy_host`.
...
- Generated configuration is validated with `nginx -t` before being promoted into the persisted `/data/nginx/conf.d/` directory.
...
- Mapping persistence is implemented through managed nginx config files in `/data/nginx/conf.d/`; those files are the only mapping store.
...
- The workflow publishes `latest` for `main`, semantic version tags such as `v0.1.2` and `0.1.2`, branch tags, and SHA tags.
- `README.md` documents image usage, Compose setup, mapping config persistence, certificate persistence, and Portainer environment URLs.
...
- A disposable container smoke test verifies health, mapping add through the real Flask app and agent socket, generated HTTPS nginx config creation under `/data/nginx/conf.d/`, `nginx -t`, mapping deletion, config removal, and config-backed mapping reads.
...
- Runtime persistence verification confirms that adding a mapping writes `/data/nginx/conf.d/<port>.conf`, recreating the container with the same `/data` volume immediately restores the nginx listener, and the UI API reads the mapping from the persisted config file.
...
- The GHCR workflow configuration is in place and publishes `main` and semantic version release tags, including the `v0.1.2` image.

## Current Next Step

Use `ghcr.io/jdeepwell/portainer-agent-proxy:0.1.2`, `ghcr.io/jdeepwell/portainer-agent-proxy:v0.1.2`, or `ghcr.io/jdeepwell/portainer-agent-proxy:latest` in the local Portainer Docker Compose stack and validate a real Portainer environment connection through `portainer_mtls_proxy:9101`.
